...
# 18 "c:\\devkitarm\\bin\\../lib/gcc/arm-eabi/4.5.0/../../../../arm-eabi/include/time.h" 3
...
void initializeGame() {
 ...
 player.cAcc = 100;
 player.stopRange = 50;
 player.maxHSpeed = ((8) << 8);
 player.maxVSpeed = ((5) << 8);
 player.isHide = 0;
...
void updatePlayer() {
 ...
 player.bigRow = player.row + ((vOff) << 8);
 player.bigCol = player.col + ((hOff) << 8);


 if(((player.row) >> 8) >= 160 - 32) {
  player.row = ((160 - 32) << 8);
  player.rDir = 0;
  jumpCount = 0;
 }
 ...
 if(((player.col) >> 8) >= 240 - 32 - 1) {
  player.col = ((240 - 32 - 1) << 8);
 }
...
void playerInput() {
 ...
 if(isAttack) {
  ...
  if((~(*(volatile unsigned int *)0x04000130) & ((1<<5)))) {
   ...
   if(hOff + 240 < 512 && ((player.col) >> 8) + player.width > 220) {
    hOff += 2;
   } else {
    player.col += ((3) << 8);
   }
  ...
  if((~(*(volatile unsigned int *)0x04000130) & ((1<<5))) && (!(~(oldButtons)&((1<<9))) && (~buttons & ((1<<9))))) {
   dash(-1);
   isDash = 1;
  } else if((~(*(volatile unsigned int *)0x04000130) & ((1<<4))) && (!(~(oldButtons)&((1<<9))) && (~buttons & ((1<<9))))) {
   dash(1);
   isDash = 1;
  }
...
MOVOBJ createEnemy() {
 ...
 newEnemy.row;
 newEnemy.col;
 newEnemy.bigRow = ((100) << 8);

 if(loc == 1) {
  newEnemy.bigCol = ((0) << 8);
 } else {
  newEnemy.bigCol = ((512) << 8);
 }
 ...
 newEnemy.maxHSpeed = ((8) << 8);
 newEnemy.maxVSpeed = ((9) << 8);
...
void updateEnemies() {
 ...
 while(nextEnemy != 0) {
  ...
  currEnemy->row = currEnemy->bigRow - ((vOff) << 8);
  currEnemy->col = currEnemy->bigCol - ((hOff) << 8);


  if(((currEnemy->bigRow) >> 8) <= 160 - 28) {
   currEnemy->bigRow = ((160 - 28) << 8);
   currEnemy->rDir = 0;
  }
     ...
     if(currEnemy->bigCol > player.bigCol) {
      currEnemy->bigCol -= ((getRand(0, 2)) << 8);
      currEnemy->flip = (1 << 12);
     }
...
void enemyCollisions(NODE* currEnemy) {
 ...
 if(((temp->bigRow) >> 8) + 32 == ((player.bigRow) >> 8) + 16) {
  if(((temp->bigCol) >> 8) >= ((player.bigCol) >> 8) && ((temp->bigCol) >> 8) <= ((player.bigCol) >> 8)) {
   if(isAttack) {
    temp->isHide = 1;
